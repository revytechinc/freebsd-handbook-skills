#!/bin/sh
# Preconditions for ports/portmaster-upgrade: pkg and git installed; the ports tree
# in /usr/ports on the branch matching pkg (as ports/ports-tree-git does it);
# portmaster installed; one installed port (sysutils/tree) older than the
...
# portmaster and tree from packages; then the ports tree is made to offer a
